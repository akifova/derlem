...
import fullMenu from '../../app-navigation';
import {onMounted, ref, watch} from 'vue';
import {useRoute, useRouter} from 'vue-router';
import role from "@/app/common/utils/role";
...
export default {
  ...
  setup(props, context) {
    ...
    for (let i = 0; i < fullMenu.length; i++) {
      let menu = {
        "text": fullMenu[i].text,
        "path": fullMenu[i].path,
        "icon": fullMenu[i].icon,
        "items": []
      };

      if (fullMenu[i].items) {
        for (let j = 0; j < fullMenu[i].items.length; j++) {
          let menuItem = fullMenu[i].items[j];
          if (role.checkPath(menuItem.path)) {
