          ...
          <DxGroupItem :col-count="3">
            ...
            <DxGroupItem :col-span="2">
              <DxGroupItem :col-count="1">
                <DxSimpleItem data-field="designPlan" :label="{text: $t('app.archive.design.designPlan')}"
                              editor-type="dxSelectBox"
                              :editor-options="designPlan">
                  <DxRequiredRule :message="$t('validation.archive.design.required-designPlan')"></DxRequiredRule>
                </DxSimpleItem>
                <DxSimpleItem data-field="creel" :label="{text: $t('app.archive.design.creelPalette')}"
                              editor-type="dxSelectBox"
                              :editor-options="creel">
                  <DxRequiredRule :message="$t('validation.archive.design.required-creel')"></DxRequiredRule>
                </DxSimpleItem>
              </DxGroupItem>
              <DxGroupItem :col-count="2">
                <DxSimpleItem data-field="quality" :label="{text: $t('app.archive.design.quality')}"
                              editor-type="dxLookup"
                              :editor-options="qualitySelectBoxOptions">
                  <DxRequiredRule :message="$t('validation.archive.design.required-quality')"></DxRequiredRule>
                </DxSimpleItem>
                <DxSimpleItem data-field="collection" :label="{text: $t('app.archive.design.collection')}"
                              editor-type="dxLookup"
                              :editor-options="collectionSelectBoxOptions">
...
import {QualityService} from "@/views/archive/design/business/QualityService";
...
import {CollectionService} from "@/views/archive/design/business/CollectionService";
...
export default {
  ...
  data() {
    return {
      ...
      creelId: '',
      designPlan: {
        dataSource: kazu.CustomStoreService('/design/plan/actives/'),
        displayExpr: (e) => {
          if (e) {
            return e.title;
          } else {
        ...
        onValueChanged: (e) => {
          if (e.value !== null) {
            this.designId = e.value.id
            this.formInstance.getEditor("creel").reset()
            this.formInstance.getEditor("creel").getDataSource().load()
          } else {
            this.formInstance.getEditor("creel").reset()
            this.formInstance.getEditor("creel").getDataSource().load()
          }
      ...
      creel: {
        dataSource: new CustomStore({
          key: 'id',
          load: () => {
            let url = '/specimen/creel/get-by-design-plan-id/' + this.designId
            return restapi({
              method: 'GET',
              url,
            })
        ...
        displayExpr: (e) => {
          if (e) {
            return e.sampleName + ' - ' + e.qualityName + ' - ' + e.paletteCode;
          } else {
            return null
          }
        },
      ...
      collectionSelectBoxOptions: CollectionService().getSelectBoxParameters(),
      sizeSelectBoxOptions: SizeService().getSelectBoxParameters(),
      shapeSelectBoxOptions: ShapeService().getSelectBoxParameters(),
      qualitySelectBoxOptions: QualityService().getSelectBoxParameters(),
        ...
        onClick: (e) => {
          ...
          if (validationResult.isValid) {
            ...
            restapi({
              ...
              this.$refs.refDesignUploader.instance.reset()
      ...
      caption: i18n.global.t('app.menu.archive.designList.text'),
    }
  },
  methods: {
